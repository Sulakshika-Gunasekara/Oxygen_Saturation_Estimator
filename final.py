...
# ==================== CONFIG ====================
DATA_DEFAULT_DIR = r"D:\RESEARCH\Oxygen_Saturation_Estimator"   # <-- your dataset folder containing *.npz
...
def cpu_latency_ms(model, n_feats, T=150, iters=50):
    ...
    # Create a new model instance on CPU to avoid moving the original model
    m_cpu = type(model)(
        n_feats=n_feats,
        d=model.d,
    ...
    m_cpu.load_state_dict(copy.deepcopy(model.state_dict()))
    
    x = torch.randn(1, T, n_feats, device=device_cpu)
    mask = torch.zeros(1, T, dtype=torch.bool, device=device_cpu)
    with torch.no_grad():
        for _ in range(10): m_cpu(x, mask)
        t0 = time.time()
        for _ in range(iters): m_cpu(x, mask)
        t1 = time.time()
...
def main():
    ...
    parser.add_argument("--train_ratio", type=float, default=0.15)  # 15% train
    parser.add_argument("--val_ratio", type=float, default=0.15)    # 15% validation
    parser.add_argument("--test_ratio", type=float, default=0.70)   # 70% test
    ...
    ds_A = NPZSeqDataset(args.data_dir, pattern="*_sequences.npz")
    N1 = len(ds_A)
    print(f"Dataset A size (N1): {N1}")
    
    # Load Dataset B: *.npz (excluding *_sequences.npz)
    print("\n=== Loading Dataset B (*.npz excluding sequences) ===")
    # Get all .npz files
    all_npz_files = sorted(glob.glob(os.path.join(args.data_dir, "**", "*.npz"), recursive=True))
    # Filter out files ending with _sequences.npz
    ds_B_files = [f for f in all_npz_files if not f.endswith("_sequences.npz")]
    
    if not ds_B_files:
        raise FileNotFoundError(f"No non-sequence .npz files found in '{args.data_dir}'")
    
    # Create a custom dataset for B
    class NPZDatasetB(Dataset):
    ...
    ds_B = NPZDatasetB(ds_B_files)
    N2 = len(ds_B)
    print(f"Dataset B size (N2): {N2}")
    
    # Verify both datasets have same number of features
    if ds_A.n_feats != ds_B.n_feats:
        raise ValueError(f"Feature mismatch: Dataset A has {ds_A.n_feats} features, Dataset B has {ds_B.n_feats} features")
    
    n_feats = ds_A.n_feats
    
    # Split Dataset A: 15% train, 15% val, 70% test
    print(f"\n=== Splitting Dataset A ===")
    idx_A_all = np.arange(N1)
    # First split: 70% test, 30% remaining (train+val)
    train_val_idx_A, test_idx_A = train_test_split(
        idx_A_all, test_size=args.test_ratio, random_state=42, shuffle=True
    )
    # Second split: 50/50 of remaining for train and val (15% each of total)
    train_idx_A, val_idx_A = train_test_split(
        train_val_idx_A, train_size=0.5, random_state=42, shuffle=True
    )
    print(f"Train from A: {len(train_idx_A)} ({len(train_idx_A)/N1*100:.1f}%)")
    print(f"Val from A:   {len(val_idx_A)} ({len(val_idx_A)/N1*100:.1f}%)")
    print(f"Test from A:  {len(test_idx_A)} ({len(test_idx_A)/N1*100:.1f}%)")
    
    # Split Dataset B: 15% train, 15% val, 70% test
    print(f"\n=== Splitting Dataset B ===")
    idx_B_all = np.arange(N2)
    # First split: 70% test, 30% remaining (train+val)
    train_val_idx_B, test_idx_B = train_test_split(
        idx_B_all, test_size=args.test_ratio, random_state=42, shuffle=True
    )
    # Second split: 50/50 of remaining for train and val (15% each of total)
    train_idx_B, val_idx_B = train_test_split(
        train_val_idx_B, train_size=0.5, random_state=42, shuffle=True
    )
    print(f"Train from B: {len(train_idx_B)} ({len(train_idx_B)/N2*100:.1f}%)")
    print(f"Val from B:   {len(val_idx_B)} ({len(val_idx_B)/N2*100:.1f}%)")
    print(f"Test from B:  {len(test_idx_B)} ({len(test_idx_B)/N2*100:.1f}%)")
    ...
    class CombinedDataset(Dataset):
        ...
        def __getitem__(self, idx):
            if idx < self.len_A:
                ...
                x, y, fi = self.ds_B[idx - self.len_A]
                # Offset file index to avoid collision
                return x, y, fi + len(self.ds_A.files)
    
    combined_ds = CombinedDataset(ds_A, ds_B)
    
    # Create combined indices
    train_idx = np.concatenate([train_idx_A, train_idx_B + N1])
    val_idx = np.concatenate([val_idx_A, val_idx_B + N1])
    test_idx = np.concatenate([test_idx_A, test_idx_B + N1])
    
    print(f"Total train samples: {len(train_idx)} (A: {len(train_idx_A)}, B: {len(train_idx_B)})")
    print(f"Total val samples:   {len(val_idx)} (A: {len(val_idx_A)}, B: {len(val_idx_B)})")
    print(f"Total test samples:  {len(test_idx)} (A: {len(test_idx_A)}, B: {len(test_idx_B)})")
    ...
    for i in train_idx:
        if i < N1:
            ...
            y_train_all.append(ds_A.ys[fi][si])
        else:
            # From dataset B
            idx_b = i - N1
            fi, si = ds_B.index[idx_b]
            y_train_all.append(ds_B.ys[fi][si])
    
    y_train_all = np.array(y_train_all, dtype=np.float32)
    y_mu = float(y_train_all.mean())
    y_std = float(y_train_all.std() + 1e-6)

    train_loader = DataLoader(Subset(combined_ds, train_idx), batch_size=BATCH, shuffle=True,
                              collate_fn=collate_pad, num_workers=num_workers, pin_memory=pin_memory)
    val_loader   = DataLoader(Subset(combined_ds, val_idx), batch_size=BATCH, shuffle=False,
                              collate_fn=collate_pad, num_workers=num_workers, pin_memory=pin_memory)
    test_loader  = DataLoader(Subset(combined_ds, test_idx), batch_size=BATCH, shuffle=False,
                              collate_fn=collate_pad, num_workers=num_workers, pin_memory=pin_memory)
    ...
    lat_ms = cpu_latency_ms(model, n_feats=n_feats, T=MAX_T, iters=50)
    ...
    row = {
        ...
        "N1": N1,
        "N2": N2,
        "Train_A": len(train_idx_A),
        "Val_A": len(val_idx_A),
        "Test_A": len(test_idx_A),
        "Train_B": len(train_idx_B),
        "Val_B": len(val_idx_B),
        "Test_B": len(test_idx_B),
    }
    out_csv = os.path.join(LOG_DIR, "results_15_15_70_dual_dataset.csv")
    ...
    plot_hardware_card(LOG_DIR, csv_name="results_15_15_70_dual_dataset.csv")

    if PLOT:
        plt.figure(figsize=(6,6))
        plt.scatter(t_true, t_pred, alpha=0.4, s=12)
        plt.xlabel("True"); plt.ylabel("Pred"); plt.title(f"Test Fit (15% Train / 15% Val / 70% Test)\nDataset A: N1={N1}, Dataset B: N2={N2}")
        plt.grid(True)
        mn = min(t_true.min(), t_pred.min())
        mx = max(t_true.max(), t_pred.max())
        plt.plot([mn, mx], [mn, mx], 'r--', linewidth=2)
        plt.savefig(os.path.join(LOG_DIR, "test_scatter_15_15_70_dual.png"), dpi=160)
        plt.close()

    print("\n" + "="*60)
    print("ACADEMIC VALIDATION COMPLETE")
    print("="*60)
    print(f"Dataset A (*_sequences.npz): N1 = {N1}")
    print(f"  Train: {len(train_idx_A)} (15%), Val: {len(val_idx_A)} (15%), Test: {len(test_idx_A)} (70%)")
    print(f"Dataset B (*.npz): N2 = {N2}")
    print(f"  Train: {len(train_idx_B)} (15%), Val: {len(val_idx_B)} (15%), Test: {len(test_idx_B)} (70%)")
    print(f"\nTotal Training:   {len(train_idx)} samples")
    print(f"Total Validation: {len(val_idx)} samples")
    print(f"Total Testing:    {len(test_idx)} samples")
